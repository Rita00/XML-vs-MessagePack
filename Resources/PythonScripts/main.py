...
def random_file(out_file, numOwners):
    ...
    for p in range(numOwners):
        ...
        phone = random.choice(OwnersPhone)
        ...
        how_many_pets = random.randint(1, 2)
        for pet in range(how_many_pets):
            ...
            weight = random.choice(PetsWeight)
        ...
        all_info.append((OwnersId[p], name, birth_owner, phone, address, pets_list))

    for p in all_info:
        f.write("%d#%s#%s#%d#%s#" % (p[0], p[1], p[2], p[3], p[4]))

        print("%d#%s#%s#%d#%s#" % (p[0], p[1], p[2], p[3], p[4]))
